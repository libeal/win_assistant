...
function Invoke-ApiTest {
    ...
    try {
        ...
        if ($UserAgent) {
            $response = Invoke-WebRequest -Uri $config.apiUrl -Method Post -Headers $Headers -UserAgent $UserAgent -Body $bodyBytes -ErrorAction Stop
        }
        else {
            $response = Invoke-WebRequest -Uri $config.apiUrl -Method Post -Headers $Headers -Body $bodyBytes -ErrorAction Stop
        }
...
# 测试1：最简单的请求
Invoke-ApiTest -Title "[测试1] 发送最简单的请求..." -Headers $baseHeaders -Body @{
    model = $config.model
    messages = @(
        @{
            role = "user"
...
# 测试2：temperature参数
Invoke-ApiTest -Title "[测试2] 测试带temperature参数的请求..." -Headers $baseHeaders -Body @{
    model = $config.model
    messages = @(
        @{
            role = "user"
...
# 测试3：模拟OpenAI SDK UA
Invoke-ApiTest -Title "[测试3] 测试带OpenAI SDK User-Agent的请求..." -Headers $baseHeaders -Body @{
    model = $config.model
    messages = @(
        @{
            role = "user"
    ...
    temperature = 0.5
    max_tokens = 100
} -UserAgent "openai-python/1.99.1"

Write-Host ""
Write-Host "=== 测试完成 ===" -ForegroundColor Cyan
Write-Host ""
Write-Host "如果三个测试都失败，可能的原因：" -ForegroundColor Yellow
